    ...
    def _decision_rows(self) -> list[dict]:
        ...
        for decision in self.engine.controller.decisions:
            ...
            rows.append({
                ...
                "reject_score": float(decision.probs[self.engine.controller.reject_mask].sum()),
                "reject_threshold": float(self.engine.policy.threshold),
                "anomaly_score": float(decision.anomaly),
                "anomaly_trigger": bool(self.engine.policy.anomaly and decision.anomaly > self.engine.model.anomaly_thresh),
