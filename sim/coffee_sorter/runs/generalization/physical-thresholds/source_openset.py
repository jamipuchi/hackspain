...
def run_sweep(args, profile: Profile, model: Model) -> dict:
    ...
    if not records or not any(r["class"] == "good" for r in records):
        raise RuntimeError("camera sweep produced no usable good/unknown objects")
...
def run_physics(args, profile: Profile, source_model: Model) -> dict:
    ...
    for i in range(n_steps):
        ...
        if i % 2 == 0:
            ...
            for decision in new_decisions:
                ...
                if unknown_uids:
                    ...
                    if decision.reject and (demo_candidate is None or
                                            sim.bean_by_uid[demo_candidate[1]].outcome in
                                            {"accept", "spilled"}):
                        demo_candidate = (decision, unknown_uids[0])
        if video:
        ...
        if video and i % video_every == 0:
            image = overview.frame("overview" if sim.data.time < args.physics_seconds * 0.65
                                   else "discharge")
            ...
            if featured:
                ...
                action = (
                    ...
                    f"{'REJECT' if decision.reject else 'KEEP'} | physics {bean.outcome or 'pending'}"
                )
            else:
            ...
            lines = [
                ...
                "HUD example selection: most recent physically rejected unknown when available",
            ]
    ...
    if video:
        ...
        rejected_examples = [(decision, uid) for decision, uid in unknown_decisions
                             if sim.bean_by_uid[uid].outcome == "reject"]
        if rejected_examples:
            demo_decision, demo_uid = rejected_examples[0]
            ...
            (args.output / "demo_unknown.json").write_text(json.dumps({
                "uid": demo_uid,
                "class": sim.bean_by_uid[demo_uid].cls,
                "anomaly_score": demo_decision.anomaly,
                "anomaly_threshold": model.anomaly_thresh,
                "classifier_reject_probability": float(
                    demo_decision.probs[controller.reject_mask].sum()),
                "combined_command": demo_decision.reject,
                "scheduled": demo_decision.scheduled,
                "activated": demo_decision.tid in sim.fired_targets,
                "jet_hit": sim.bean_by_uid[demo_uid].jet_hits > 0,
                "physical_outcome": sim.bean_by_uid[demo_uid].outcome,
                "selection": "first unknown linked to a decision that physically reached reject",
            }, indent=1))
    result = {
